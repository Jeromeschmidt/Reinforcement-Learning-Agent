# -*- coding: utf-8 -*-
"""
Alpaca API - MACD + stochastic strategy
...
tickers = "FB,XM,SAP,LNSR,FSLR,NIO,LMND,FTCH,PSTH,DASH,SBE,PFE,CAKE,SPCE,RVLV,FTCH,AMZN,INTC,PLTR,ELY,CCIV,MSFT,AAPL,GOOG,CSCO,CMCSA,ADBE,NVDA,NFLX,PYPL,AMGN,AVGO,TXN,CHTR,QCOM,GILD,FISV,BKNG,INTU,ADP,CME,TMUS,MU"
...
message['Subject'] = 'Alpaca MACD & Stochastic Trading Algorithm'   #The subject line
...
def MACD(df_dict, a=12 ,b=26, c=10):
...
def main():
    global stoch_signal
    historicalData = hist_data(tickers, "5Min")
    MACD(historicalData)
    ...
    for ticker in tickers.split(","):
        ...
        for position in positions:
            if len(positions) > 0:
                if position.symbol == ticker and position.qty !=0:
                    ...
                    existing_pos = True
                    # mail_content = "Trades have been made, bought stock"
        ...
        if historicalData[ticker]["macd"].iloc[-1]> historicalData[ticker]["signal"].iloc[-1] and \
            ...
            stoch_signal[ticker]=="oversold" and existing_pos == False:
                api.submit_order(symbol=ticker, qty=max(1,int(max_pos/historicalData[ticker]["close"].iloc[-1])), side="buy",type="market", time_in_force="ioc")
                print("bought {} stocks in {}".format(int(max_pos/historicalData[ticker]["close"].iloc[-1]),ticker))
                mail_content = "bought {} stocks in {}".format(int(max_pos/historicalData[ticker]["close"].iloc[-1]),ticker)
                ...
                try:
                    ...
                    time.sleep(1)
                    api.submit_order(symbol=ticker, qty=int(filled_qty), side="sell", type="trailing_stop", time_in_force="day", trail_percent = "1.5")
                    mail_content = "Trades have been made, stock sold"
                    #The body and the attachments for the mail
                    message.attach(MIMEText(mail_content, 'plain'))
                    #Create SMTP session for sending the mail
                    session = smtplib.SMTP('smtp.gmail.com', 587) #use gmail with port
...
def run():
    ...
    starttime = time.time()
    timeout = starttime + 60*60*8

    clock = api.get_clock()

    if clock.is_open != True:
        mail_content = "The Market is Closed Today. I can't place any trades right now Andre."
        #The body and the attachments for the mail
        message.attach(MIMEText(mail_content, 'plain'))
        #Create SMTP session for sending the mail
        session = smtplib.SMTP('smtp.gmail.com', 587) #use gmail with port
    ...
    #     time.sleep(5)
    #         #The body and the attachments for the mail
    #     mail_content = 'Finished Trading Today. Closed All Positions'
    #     message.attach(MIMEText(mail_content, 'plain'))
    #     #Create SMTP session for sending the mail
    #     session = smtplib.SMTP('smtp.gmail.com', 587) #use gmail with port
    #     session.starttls() #enable security
    ...
    #     session.quit()
        ...
        #The body and the attachments for the mail
        mail_content = 'Finished Trading Today. I Took No Positions'
        message.attach(MIMEText(mail_content, 'plain'))
        #Create SMTP session for sending the mail
        session = smtplib.SMTP('smtp.gmail.com', 587) #use gmail with port
        session.starttls() #enable security
